...
assign final_fifo_out_valid_internal = `gathered_elected_direct_message_valid;
...
generate
    for (i=0; i < TRUE_FIFO_COUNT; i=i+1) begin: taking_direct_message
        assign `master_coming_is_taken(i) = 
            ((i == `gathered_elected_direct_message_index) && `gathered_elected_direct_message_valid  && !final_fifo_out_is_full_internal);
    end
endgenerate
